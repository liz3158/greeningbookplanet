...
<head>
    ...
    <link rel="stylesheet" href="css/order_track_underline.css">
    <!--下划线<link href='https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css' rel='stylesheet'>-->
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6 col-md-4 col-sm-6 col-xs-12 ">
                        <div class="header__actions"></span><a href="login.php">Login & Register</a>
                            <div class="btn-group ps-dropdown"><a class="dropdown-toggle" href="index.php" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Status</a>
            ...
            <div class="card-body">
                <article class="card">
                    <div class="card-body row">
                        <div class="col"> <strong><span id="book_name"></span></strong> <br>
                            <span id="book_price"></span> <br>
                            <span id="shipping_price"></span> <br>
                            <span id="total_price"></span><br>
                        </div>
                    </div>
                </article>
                <div class="track">
                    <div class="step" id="order_status_1"> <span class="icon"> <i class="fa fa-check"></i> </span> <span class="text">Order confirmed</span> </div>
                    <div class="step" id="order_status_2"> <span class="icon"> <i class="fa fa-truck"></i> </span> <span class="text"> In Delivery </span> </div>
                    <div class="step" id="order_status_3"> <span class="icon"> <i class="fa fa-box"></i> </span> <span class="text">Order Received</span> </div>
                </div>
                <hr>
                <br>

                <div align="center">
                    <button class="ps-btn" id="btnreceive" onclick="receiveorder('<?php echo $_GET["orderid"]; ?>','<?php echo $_GET["book_id"]; ?>');">Receive Order</button>
                    <br><br>
                    <hr> <a href="track_order.php" data-abc="true"> <i class="fa fa-chevron-left"></i> Back to orders</a>
                </div>
...
<script>
    ...
    function getBookDetail() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#book_name").html("<strong>Book Name : </strong>" + response);
            }
        });
    }
    ...
    function getBookPrice() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#book_price").html("<strong>Book Price : </strong> RM " + parseFloat(response).toFixed(2));
            }
        });
    }
    ...
    function getShippingPrice() {
        ...
        $.ajax({
            ...
            success: function(response) {
                totalprice += parseFloat(response);
                $("#shipping_price").html("<strong>Shipping Price :</strong>RM " + parseFloat(response).toFixed(2));
            }
        });
    }
    ...
    function getorderstatus() {
        ...
        $.ajax({
            ...
            async: false,
            success: function(response) {
                var orderstatus = response;
                if (orderstatus != "pending") {
                    //order_status_1 order_status_2 order_status_3

                    if (orderstatus == "orderconfirmed") {
                        $("#order_status_1").addClass("active");

                    }
                    if (orderstatus == "delivery") {
                        $("#order_status_1").addClass("active");
                        $("#order_status_2").addClass("active");

                    }
                    if (orderstatus == "received") {
                        $("#order_status_1").addClass("active");
                        $("#order_status_2").addClass("active");
                        $("#order_status_3").addClass("active");
                        $("#btnreceive").addClass("d-none");
                    }
                }
                if (orderstatus == "pending") {
                    $("#btnreceive").addClass("d-none");
    ...
    function orderviewaccess() {
        $.ajax({
            ...
            success: function(response) {
                ...
                var book_user_id = response;
                if (user_id != book_user_id) {
                    location.href = "track_order.php";
                }
            }
        });
    }

    function receiveorder(orderid, book_id) {
        $.ajax({
            url: "infotracker.php",
            data: {
            ...
            success: function(response) {
                ...
                if (response == "ok") {
                    ...
                    location.href = "track_info.php?orderid=" + orderid + "&book_id=" + book_id;
    ...
    getShippingPrice();
    getorderstatus();
    $("#total_price").html("<strong>Total Price :</strong>RM " + totalprice.toFixed(2));
</script>