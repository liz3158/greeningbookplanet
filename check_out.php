    ...
    <nav class="navigation">
      <div class="container-fluid">
        ...
        <div class="navigation__column right">
          <div class="ps-cart"><a class="ps-cart__toggle" href="#"><span></span><i class="ps-icon-shopping-cart"></i></a>
          </div>
        ...
        <div class="row">
          <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12 ">
            <div class="ps-checkout__billing">
              <h3>Confirmation Of Billing Detail</h3>
              <label>Notes: Update Your address at Personal Information</label>
              <br><br><br>
              <div class="form-group form-group--inline">
                <label>Name<sub style="color: red;">*</sub>
                </label>
                <input class="form-control" type="text" name="user_name" value="<?php echo $user_name; ?>" readonly="readonly">
              </div>
              <div class="form-group form-group--inline">
                <label>Address<sub style="color: red;">*</sub>
                </label>
                <textarea type="text" class="form-control" name="user_address" readonly="readonly"><?php echo $user_address; ?></textarea>
              </div>
              <div class="form-group form-group--inline">
                <label>State<sub style="color: red;">*</sub>
                </label>
                <input class="form-control" type="text" name="user_state" value="<?php echo $user_state; ?>" readonly="readonly">
              </div>
              <div class="form-group form-group--inline">
                <label>City<sub style="color: red;">*</sub>
                </label>
                <input class="form-control" type="text" name="user_city" value="<?php echo $user_city; ?>" readonly="readonly">
              </div>
              <div class="form-group form-group--inline">
                <label>Zipcode<sub style="color: red;">*</sub>
                </label>
                <input class="form-control" type="text" name="user_zipcode" value="<?php echo $user_zipcode; ?>" readonly="readonly">
              </div>
          ...
          <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 ">
            <div class="ps-checkout__order">
                ...
                <table class="table ps-checkout__products">
                  <thead>
                    <tr>
                      ...
                      <th class="text-uppercase">Shipping Price</th>
                    </tr>
                  </thead>
                  <tbody id="tabledata">
              ...
              <footer>
                ...
                <div class="form-group cheque">
                  <div class="ps-radio">
                    <input class="form-control" type="radio" id="rdo01" name="payment" value = "onlinebank" checked>
                    <label for="rdo01">Online Banking​</label>
                    <p></p>
                  </div>
                </div>
                <div class="form-group paypal">
                  <div class="ps-radio ps-radio--inline">
                    <input class="form-control" type="radio" name="payment" id="rdo02"  value = "cdm">
                    <label for="rdo02">Cash Deposit Machine(CDM)</label>
                  </div>

                </div>
                <div class="form-group paypal" style="color: aliceblue;">
                  <h5 style="color:white">Please send your PDF file receipt</h5><br>
                  <input class="form-control-file" type="file" id="filepicker" name="" >
                  <!--<label for="">PDF</label>-->
                </div>

                <div class="form-group">
                  <div class="ps-btn--fullwidth">
                    <button class="ps-btn ps-btn--fullwidth"  id="purchasebtn" type="button" onclick="placeorder();" disabled>Place Order<i class="ps-icon-next"></i></button>
                  </div>
                </div>
            ...
            <div class="ps-shipping" style="border-style: groove; border-width: 5px; border-color: black; background-color: lightgrey; border-radius: 20px; box-shadow: 5px 10px #888888;">
              <h2 style="color:red;">GBP Bank Details​</h3><br>
              <p style="color:red;"><b>Receiver Name/Company :​Greening Book Planet<br></b></p>
              <p style="color:red;"> <b> Bank Name:Public Bank<br></b></p>
              <p style="color:red;"><b>  Acc No​ :6488509814<br><br></b></p>
              <p style="color:red;"> <b> **For reference please write down your email<br>**Please upload Receipt(proof) in pdf only​</b></p>
            </div>
          </div>
        </div>
      </form>
...
<script>
   ...
   var shippingPrice = 0.0;
    ...
    $.ajax({
                  ...
                  // alert(response);
                    ...
                    for (var x = 0; x < json.length; x++) {
                      ...
                      //  var book_quantity = json[x].book_quantity;
                        ...
                        getshippingprice(seller_id,user_id);
                      totalprice +=parseFloat(book_price)+parseFloat(shippingPrice);
                        html+='<tr>'+
                               '<td>'+book_name+'</td><td>RM '+parseFloat(book_price).toFixed(2)+'</td>'+
                               '<td>RM '+parseFloat(shippingPrice).toFixed(2)+'</td>'+
                               '</tr>';
                    }
                       html +='<tr><td colspan="2">Order Total</td><td>RM '+totalprice.toFixed(2)+'</td></tr>';
                    $("#tabledata").html(html);
                }
      ...
      function isfilechoosed(){
        if(!filepicked){
          if($("#filepicker").val()!=""){
              $("#purchasebtn").prop("disabled",false);
          filepicked = true;
        }
        setTimeout(function(){isfilechoosed();},1000);
        }
...
<script>
  ...
  function placeorder(){
      ...
      if(data_blob!=""){
          var new_pdf_name = Math.floor(Math.random() * (999999 - 10000 + 1) + 10000).toString()+".pdf";
          filepath  = "/images/receipt/"+new_pdf_name;
        uploadReceiptPayment(data_blob,new_pdf_name);

      }
      var payment_method = $("input[name='payment']:checked").val();
     
      for (var x = 0; x < cart.length; x++) {
            var cart_id=cart[x].cart_id;
            var book_id = cart[x].book_id;
            var seller_id_ = cart[x].seller_id;
            makeorder(user_id,seller_id_,payment_method,filepath,book_id,cart_id);
        }
      location.href = "track_order.php?orderid="+order_id;
  ...
  function setorderitem(order_id,book_id){
    $.ajax({
            url: "placeorder.php",
            data: {
  ...
  function makeorder(user_id,seller_id,payment_method,filepath,book_id,cart_id){
    $.ajax({
            url: "placeorder.php",
            data: {
...
function uploadReceiptPayment(receiptdata,receipt){
    $.ajax({
