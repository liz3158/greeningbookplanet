...
function getbookinfo($book_id, $orderid)
{
  global $conn;
  ...
  if (mysqli_num_rows($result) > 0) {
    while ($rows = mysqli_fetch_assoc($result)) {
      ...
      $html .= '
      <tr>
      <td><a class="ps-product__preview" href="product_detail.php?book_id=' . $book_id . '"><img class="mr-15" src="seller/img_book/' . $rows["img1"] . '" width="80" height="120" alt=""> ' . $rows["book_name"] . '</a></td>
      <td>
        <p>' . $rows["book_author"] . '</p>
      </td>
      <td>
        <p>RM' . $rows["book_price"] . '</p>
      </td>
      <td>
        <a href="track_info.php?orderid=' . $orderid . '&book_id=' . $book_id . '"><button class="ps-btn">Track<i class="ps-icon-next"></i></button></a>
        ...
        <table class="table ps-cart__table">
          ...
          <tbody>
            ...
            if (mysqli_num_rows($result) > 0) {
              while ($rows = mysqli_fetch_assoc($result)) {
                //  echo $orderid;
                if ($rows["order_status"] == "delivery") {
                  getbookinfo($rows["book_id"], $rows["order_id"]);
                } elseif ($rows["order_status"] == "orderconfirmed") {
                  getbookinfo($rows["book_id"], $rows["order_id"]);
                } elseif ($rows["order_status"] == "pending") {
                  getbookinfo($rows["book_id"], $rows["order_id"]);
                } elseif ($rows["order_status"] == "received") {
                  if ($rows["admin_bank_in_status"] == "pending") {
                    getbookinfo($rows["book_id"], $rows["order_id"]);
                  }
                }
              }
